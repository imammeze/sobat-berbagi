...
class EventController extends Controller
{
    ...
    {
        $campaigns = $this->campaignRepository->getCampaignsByStatus('verified', 10, request()->category, request()->search, 'event');
        $categories  = $this->categoryRepository->getAllCampaignCategories();

        return view('pages.frontend.event.index', compact('campaigns', 'categories'));
    }
    ...
    public function show(Campaign $campaign)
    {
        ...
        return view('pages.frontend.event.show', compact('campaign', 'donations'));
    }
}
